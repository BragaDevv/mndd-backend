...
async function buscarDevocionalPorData(data: Date) {
  ...
  try {
    ...
    const imagem = $("img").first().attr("src") || null;

    const conteudo = $("article").text().trim(); // tentativa ampla

    // ⚠️ Mesmo se falhar, retornar dados básicos com o link
    return {
      titulo: titulo || "Devocional do Dia",
      conteudo: conteudo || "",
      imagem,
